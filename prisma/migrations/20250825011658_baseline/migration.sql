...
EXCEPTION WHEN duplicate_object THEN NULL;
END$$;
ALTER TYPE "public"."CommunityJoinPolicy" ADD VALUE IF NOT EXISTS 'OPEN';
ALTER TYPE "public"."CommunityJoinPolicy" ADD VALUE IF NOT EXISTS 'INVITE_ONLY';
ALTER TYPE "public"."CommunityJoinPolicy" ADD VALUE IF NOT EXISTS 'DOMME_ONLY';
ALTER TYPE "public"."CommunityJoinPolicy" ADD VALUE IF NOT EXISTS 'SUB_ONLY';
...
-- ===================================================
-- Column guards (idempotent; falls Tabellen schon alt)
-- ===================================================
ALTER TABLE "public"."Post"      ADD COLUMN IF NOT EXISTS "communityId" TEXT;
ALTER TABLE "public"."Post"      ADD COLUMN IF NOT EXISTS "repostOfId"  TEXT;
ALTER TABLE "public"."Post"      ADD COLUMN IF NOT EXISTS "quoteOfId"   TEXT;
ALTER TABLE "public"."Community" ADD COLUMN IF NOT EXISTS "joinPolicy"  "public"."CommunityJoinPolicy" NOT NULL DEFAULT 'OPEN';
ALTER TABLE "public"."Community" ADD COLUMN IF NOT EXISTS "bannerUrl"   TEXT;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Post_repostOfId_fkey') THEN
    ALTER TABLE "public"."Post"
      ADD CONSTRAINT "Post_repostOfId_fkey" FOREIGN KEY ("repostOfId") REFERENCES "public"."Post"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'Post_quoteOfId_fkey') THEN
    ALTER TABLE "public"."Post"
      ADD CONSTRAINT "Post_quoteOfId_fkey" FOREIGN KEY ("quoteOfId") REFERENCES "public"."Post"("id") ON DELETE SET NULL ON UPDATE CASCADE;
  END IF;
END$$;
