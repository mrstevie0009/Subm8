...
-- UNIQUE auf lastMessageId (falls noch nicht vorhanden)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- FK für lastMessageId -> Message(id)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'Conversation_lastMessageId_fkey'
  ) THEN
    ALTER TABLE "public"."Conversation"
...
-- 2) Conversation: schnelle Chatliste
