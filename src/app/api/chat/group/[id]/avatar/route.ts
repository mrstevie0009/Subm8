...
export async function PUT(req: Request, { params }: Ctx) {
  ...
  if (!membership || membership.conversation?.type !== $Enums.ConversationType.GROUP) {
  ...
  if (!/^image\//.test(type)) {
    return Response.json({ ok: false, error: 'Unsupported type' }, { status: 400 });
  }

  // Upload nach Storage
    const storage = getStorage();

    // kleine Helfer: Extension aus MIME ableiten
    const ext = type.split('/')[1]?.toLowerCase() || 'png';
    // eindeutiger Key im erlaubten Bucket "avatars"
