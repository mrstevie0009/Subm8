...
export default function AutoDrainEnableModal({
  ...
  React.useEffect(() => {
    if (!open) return;
    // Reset + Disclaimer-Ack aus localStorage
    setAmount('50');
    setCadence('MONTHLY');
    setError(null);
    try {
  ...
  async function handleEnable() {
    ...
    try {
      ...
      // Autodrain direkt aktivieren (wie im Accept-Modal)
      const res = await fetch('/api/autodrain/accept', {
        ...
        body: JSON.stringify({
          ...
          currency,
          cadence,
          // conversationId absichtlich weggelassen – Server kann optional zuordnen.
        }),
      });
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              disabled={sending || !giftAck || !amountValid}
              className={`px-4 py-2 rounded-lg text-white transition ${
                giftAck && amountValid && !sending ? 'bg-[var(--purple)] hover:opacity-95' : 'bg-white/10 opacity-60 cursor-not-allowed'
              }`}
            >
