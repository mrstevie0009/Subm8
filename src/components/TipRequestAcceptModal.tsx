...
export default function TipRequestAcceptModal({
  ...
  async function handleAccept() {
    try {
      ...
      if (!res1.ok || !j1?.ok || !j1.paymentId) throw new Error(j1?.error || 'Could not create payment');
      ...
      <div className="relative w-[min(640px,94vw)] rounded-2xl overflow-hidden border border-white/10 bg-[#0b0b0d]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="relative px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="font-semibold text-[16px] leading-tight truncate">Accept gift request</div>
        ...
        <div className="px-5 pb-5">
          ...
          <div className="rounded-xl border border-white/10 bg-white/[.03] p-3">
            ...
            <p className="mt-1 text-[12px] text-white/60">The creator sees this amount. A 10% platform fee is added to what you pay.</p>
          </div>
          ...
          <div className="mt-4 flex items-center justify-end gap-2">
            ...
            <button
              ...
              disabled={sending}
              className={`relative px-4 py-2 rounded-lg text-white transition ${!sending ? 'bg-[var(--purple)] hover:opacity-95' : 'bg-white/10 opacity-60 cursor-not-allowed'}`}
            >
              {sending ? 'Processing…' : `Send Gift & Accept (${fmtCurrency(totalCents, currency)})`}
            </button>
          </div>
        </div>
      </div>
